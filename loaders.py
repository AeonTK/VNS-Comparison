...
def load_graph_from_csv(stations_csv, edges_csv):
    """
    Loads a graph from two csv files, one containing the stations and one containing the edges.
    :param stations_csv: path to the csv file containing the stations
    :param edges_csv: path to the csv file containing the edges
    :return: a NetworkX graph representation of the data
    """
    graph = nx.DiGraph()
    node_data = {}
    ...
    for _, row in stations_df.iterrows():
        # id,h3_id,pos,capacity,target,load,is_depot,index
        # 1000,8a1f8d70275ffff,"(48.1707661, 11.7166837)",15,7,7,True,0
        pos = row['pos'][1:-1].split(', ')
        node_data[str(row['index'])] = {'bin_id': row['id'], 'pos': (float(pos[1]), float(pos[0]))}
        graph.add_node(str(row['index']), sup=-row['target'] + row['load'])
        graph.add_edge(str(row['index']), str(row['index']), dist=0)

    edges_df = pd.read_csv(edges_csv)
    edges = edges_df.to_dict(orient="records")

    for edge in edges:
        graph.add_edge(str(edge['source']), str(edge['target']), dist=edge['distance'])
