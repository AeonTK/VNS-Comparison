...
class ProblemInstance:
    ...
    def _initialize_tracking_variables(self, input_graph: nx.Graph):
        ...
        if self.total_sink == self.total_source:
            ...
            # self.imbalance = self.total_source + self.total_sink
    ...
    def display_results(self, show_instructions=True):
        ...
        if show_instructions:
            ...
            for v in self.vehicles:
                ...
                line = "\nVehicle #{:<3} {:>12}km |".format(v.id(), round(dist, 3))
                prev_load, last = 0, -1
        ...
        d = round(self.calculate_costs(), 3)
        dff = round(self.calculate_costs(use_ff_ratio=False), 3)
        ...
        results += bcolors.BOLD + bcolors.OKGREEN + "Total Distance:{:10}km ({:8}km)".format(d, dff) + bcolors.ENDC
